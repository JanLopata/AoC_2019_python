import unittest

import day09 as day

test_data_part1 = """
109,2000,109,19,11102,1,102030,1985,204,-34,99;102030
109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99;109,1,204,-1,1001,100,1,100,1008,100,16,101,1006,101,0,99
1102,34915192,34915192,7,4,7,99,0;1219070632396864
104,1125899906842624,99;1125899906842624
"""

test_data_part2 = """
3,26,1001,26,-4,26,3,27,1002,27,2,27,1,27,26,27,4,27,1001,28,-1,28,1005,28,6,99,0,0,5;139629729
3,52,1001,52,-5,52,3,53,1,52,56,54,1007,54,5,55,1005,55,26,1001,54,-5,54,1105,1,12,1,53,54,53,1008,54,0,55,1001,55,1,55,2,53,55,53,4,53,1001,56,-1,56,1005,56,6,99,0,0,0,0,10;18216
"""


class AoCTest(unittest.TestCase):
    def test_part1(self):
        for line in test_data_part1.splitlines():
            if line == "":
                continue
            result = day.part1(line.split(";")[0])
            interpreted_result = ",".join(str(it) for it in result)
            self.assertEqual(line.split(";")[1], interpreted_result)

    def test_part2(self):
        for line in test_data_part2.splitlines():
            if line == "":
                continue
            result = day.part2(line.split(";")[0])
            self.assertEqual(int(line.split(";")[1]), result)


if __name__ == '__main__':
    unittest.main()
